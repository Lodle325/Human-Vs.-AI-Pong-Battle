...
function runOneGeneration() {
  ...
  brains.forEach(function(brain) {
    var hits = 0, misses = 0, totalMissDist = 0;
  ...
  // Sort best to worst
  brains.sort(function(a, b) { return b.fitness - a.fitness; });
...
function drawGame(ctx) {
  ...
  if (state === 'waiting') {
    ...
    if (generation === 0) {
      ...
      ctx.fillText('AI is untrained — beat it before it learns!', W / 2, H / 2 + 44);
    }
  }
  ...
  if (state === 'scored') {
    ...
    ctx.fillText(lastScorer === 'player' ? 'YOU SCORED' : 'AI SCORED', W / 2, H / 2 - 14);
